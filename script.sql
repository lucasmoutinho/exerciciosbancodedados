...
SELECT empresa.empregado.nome, endereco FROM empresa.empregado INNER JOIN empresa.departamento
...
SELECT nome FROM empresa.empregado ORDER BY nome ASC;

--15 Listar nomes e horas trabalhadas por empregados no projeto de código 3,
-- em ordem decrescente de horas trabalhadas.

SELECT empresa.empregado.nome, empresa.trabalhaem.horas FROM empresa.empregado INNER JOIN empresa.trabalhaem
...
SELECT DISTINCT ON (codemp) codemp FROM empresa.trabalhaem WHERE horas > 10;
...
FROM empresa.trabalhaem GROUP BY codproj ORDER BY codproj ASC;

-- 19 Obter os códigos de projetos cuja média de horas trabalhadas seja maior
-- que 20. Dica: use HAVING.

SELECT codproj FROM empresa.trabalhaem GROUP BY codproj HAVING AVG(horas) > 20;
...
WHERE empresa.projeto.codproj IN (SELECT codproj FROM empresa.trabalhaem GROUP BY codproj 
HAVING AVG(horas) > 20 );